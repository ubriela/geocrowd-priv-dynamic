...
def neighbors_dir(node, direction, neighbor):
    """
    Find all neighbors (leaf nodes) of a node based on a direction
    neighbor is in the first level
    """
    _neighbors = []
    if neighbor.n_isLeaf or neighbor.children is None:
        _neighbors.append(neighbor)
    else:
        gran = len(neighbor.children)
        if direction == "N":
            rng = [node.n_box[0, 0], node.n_box[1, 0]]
            for i in range(gran):
                m = neighbor.children[i][gran - 1]
                range_neighbor = [m.n_box[0, 0], m.n_box[1, 0]]
                if is_range_overlap(rng, range_neighbor):
                    _neighbors.append(m)
        elif direction == "S":
            rng = [node.n_box[0, 0], node.n_box[1, 0]]
            for i in range(gran):
                m = neighbor.children[i][0]
                range_neighbor = [m.n_box[0, 0], m.n_box[1, 0]]
                if is_range_overlap(rng, range_neighbor):
                    _neighbors.append(m)
        elif direction == "W":
            rng = [node.n_box[0, 1], node.n_box[1, 1]]
            for j in range(gran):
                m = neighbor.children[gran - 1][j]
...
def neighbors(tree, node):
    ...
    parent = node.parent
    if node.n_depth == 2:  # 2nd level
        gran_2nd = len(parent.children)
        i, j = node.index / gran_2nd, node.index % gran_2nd
        k, l = parent.index / gran_1st, parent.index % gran_1st
        if j == 0:
            q_neighbors.append(parent.children[i][j + 1])
            if i == 0:  # [0,0]
                q_neighbors.append(parent.children[i + 1][j])
                if l >= 1:
                    q_neighbors.extend(neighbors_dir(node, "N", tree.root.children[k][l - 1]))
                if k >= 1:
                    q_neighbors.extend(neighbors_dir(node, "W", tree.root.children[k - 1][l]))
            elif i == gran_2nd - 1:  # [2,0]
                q_neighbors.append(parent.children[i - 1][j])
                if l >= 1:
                    q_neighbors.extend(neighbors_dir(node, "N", tree.root.children[k][l - 1]))
                if k <= gran_1st - 2:
                    q_neighbors.extend(neighbors_dir(node, "E", tree.root.children[k + 1][l]))
            else:  # [1,0]
                q_neighbors.append(parent.children[i - 1][j])
                q_neighbors.append(parent.children[i + 1][j])
                if l >= 1:
            ...
            if i == 0:  # [0,2]'
                q_neighbors.append(parent.children[i + 1][j])
                if l <= gran_1st - 2:
                    q_neighbors.extend(neighbors_dir(node, "S", tree.root.children[k][l + 1]))
                if k >= 1:
                    q_neighbors.extend(neighbors_dir(node, "W", tree.root.children[k - 1][l]))
            ...
            if i == 0:  # [0,1]
                ...
                q_neighbors.append(parent.children[i - 1][j])
                q_neighbors.append(parent.children[i + 1][j])
    elif node.n_depth == 1:  # 1st level
        i, j = node.index / gran_1st, node.index % gran_1st
        if j == 0:
            q_neighbors.extend(neighbors_dir(node, "S", parent.children[i][j + 1]))
            if i == 0:  # [0,0]
                q_neighbors.extend(neighbors_dir(node, "E", tree.root.children[i + 1][j]))
            elif i == gran_1st - 1:  # [2,0]
                q_neighbors.extend(neighbors_dir(node, "W", tree.root.children[i - 1][j]))
            else:  # [1,0]
                q_neighbors.extend(neighbors_dir(node, "E", tree.root.children[i + 1][j]))
                q_neighbors.extend(neighbors_dir(node, "W", tree.root.children[i - 1][j]))
        elif j == gran_1st - 1:
            q_neighbors.extend(neighbors_dir(node, "N", parent.children[i][j - 1]))
            if i == 0:  # [0,2]
                q_neighbors.extend(neighbors_dir(node, "E", tree.root.children[i + 1][j]))
...
def leaf_granularity(tree, index):
    return len(tree.root.children[index[0]][index[1]])
...
def geocast(tree, t, eps, i):
    ...
    q_init.l_count[i] = math.floor(q_init.l_count[i])
